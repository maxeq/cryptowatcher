...
const useBinanceTicker = () => {
  const [cryptocurrenciesBinance, setCryptocurrencies] = useState(
    (typeof window !== 'undefined' &&
      JSON.parse(localStorage.getItem(BINANCE_TICKER_STORAGE_KEY))) ||
      CRYPTOCURRENCIES
  );
    ...
    try {
      ...
      setCryptocurrencies(
        cryptocurrenciesBinance.map(item => {
          const cryptoBinanceData = data.find(
            cryptoBinance => cryptoBinance.symbol === item.symbol
          );
          return {
            ...item,
  ...
  useEffect(() => {
    const interval = setInterval(fetchCrypto, 300000);
  ...
  useEffect(() => {
    localStorage.setItem(
      BINANCE_TICKER_STORAGE_KEY,
      JSON.stringify(cryptocurrenciesBinance)
    );
  }, [cryptocurrenciesBinance]);
